...
const GROUPS = {
    ...
    groupDetails: async function (id, token, callback) {
        ...
        let groupMembers = await DB.query('select iduser as id, username from users where iduser in (select iduser from user_groups where idgroup = $1 and accepted = true) order by username;', [id]);
